...
const double heat_diff_constant = 0.005;
...
const int N_p = 2;
//const int number_of_time_steps = 500;
...
std::pair<vec_x<T>, jacobian_x_p<T>> adjoint_evolution (int n, vec_x<T> x0, const vec_p<PT> p0) {
    ...
    /*
    DCO_T x_in_0; dco::value(x_in_0)=std::get<0>(x0);
    DCO_T x_in_1; dco::value(x_in_1)=std::get<1>(x0);

    DCO_T p_in_0; dco::value(p_in_0)=std::get<0>(p0);
    DCO_T p_in_1; dco::value(p_in_1)=std::get<1>(p0);
    ...
    */
	...
	for (int k2=0; k2<N_p;k2++) {
	  (jac.second)[i][k2] = dco::derivative(p_in[k2]);
        ...
        /*
	(jac.first)[i][0] = dco::derivative(x_in[0]);
        (jac.first)[i][1] = dco::derivative(x_in[1]);
        (jac.second)[i][0] = dco::derivative(p_in[0]);
        (jac.second)[i][1] = dco::derivative(p_in[1]);
	*/
...
int main(int argc, char* argv[]) {
  ...
  const int number_of_time_steps = (int) strtol(argv[2], NULL, 10);
  std::cout << number_of_time_steps << std::endl;
    vec_x<double> zeros;
    zeros.fill(0.0);
    ...
    //std::string file_name = strcat(name, argv[1]); 
    out_stream.open("data" + (std::string)argv[1] + ".txt");
...
// printf("} \n ");
    ...
    // out_stream << "Print gradients:" << std::endl;
    //for (int j=0; j<N_x; j++) {
    //out_stream << dydx[100][j] << ' ';
